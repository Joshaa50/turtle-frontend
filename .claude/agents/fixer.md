...
## Hard rules
- NEVER edit anything under `tests/`, any `*.test.ts(x)` file, or `vitest.config`/`vite.config` test setup.
...
- NEVER claim the suite passes. You do not run the QA gate; you may run a single focused test
  (`npx vitest run <path> -t "<name>"`) to check your own reasoning while working.

## Procedure
1. Read `turtle-frontend/qa-report.json`. Work failures in order: typecheck, then tests, then build.
...
4. If a failure is beyond a narrow fix (needs a design decision, a schema change, or touching the
   backend contract), do NOT guess. Skip it, and list it as `needs-decision` with your reasoning.
